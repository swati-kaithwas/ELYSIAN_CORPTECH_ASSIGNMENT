...
const login = async(req,res)=>{
    ...
    //   check if user is exist
        ...
        return res.send({
            status:false,
            messaage:"Either email or password incorrect"
        })
        // apply jwt token
...
const logout =async(req,res)=>{
    try{
        ...
        */
        ...
        if (!isLoggedOut)
    return res.status(400) .json({ status: false, message: "Unable to logout." });
         res.status(200).json({ status: true, message: "successfully logged out" });
        } catch (error) {console.log("error in logout api: ", error);
